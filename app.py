...
st.markdown("""html, body, [class*="css"]  { direction: rtl; text-align: right; font-size: 18px; }h1, h2, h3, h4, h5, h6 { direction: rtl; text-align: right; }.stDataFrame, .stTable { direction: rtl; }label, p, div { direction: rtl; }section[data-testid="stSidebar"] { direction: rtl; }.block-container { padding-top: 1.2rem; }""",unsafe_allow_html=True)
...
def get_pg_conn():if "db" not in st.secrets or "url" not in st.secrets["db"]:st.error("חסר Secrets: db.url (Streamlit Settings → Secrets)")st.stop()return psycopg2.connect(st.secrets["db"]["url"])

def init_pg():conn = get_pg_conn()cur = conn.cursor()cur.execute("""CREATE TABLE IF NOT EXISTS schedules (id BIGSERIAL PRIMARY KEY,username TEXT NOT NULL,customer_name TEXT NOT NULL,week INT NOT NULL,day TEXT NOT NULL,shift TEXT NOT NULL,worker TEXT NOT NULL,created_at TIMESTAMPTZ DEFAULT NOW());""")cur.execute("""CREATE INDEX IF NOT EXISTS idx_schedules_user_weekON schedules(username, customer_name, week);""")conn.commit()cur.close()conn.close()

def upsert_week_schedule(username: str, customer_name: str, week: int, df: pd.DataFrame):"""דריסה לפי: username + customer_name + weekמוחק מה שהיה לשבוע הזה ומכניס חדש."""conn = get_pg_conn()cur = conn.cursor()
...
def list_weeks(username: str, customer_name: str | None = None):conn = get_pg_conn()cur = conn.cursor()if customer_name:cur.execute("""SELECT DISTINCT weekFROM schedulesWHERE username=%s AND customer_name=%sORDER BY week DESC""",(username, customer_name))else:cur.execute("""SELECT DISTINCT weekFROM schedulesWHERE username=%sORDER BY week DESC""",(username,))weeks = [int(x[0]) for x in cur.fetchall()]cur.close()conn.close()return weeks

def list_customers(username: str):conn = get_pg_conn()cur = conn.cursor()cur.execute("""SELECT DISTINCT customer_nameFROM schedulesWHERE username=%sORDER BY customer_name ASC""",(username,))customers = [x[0] for x in cur.fetchall()]cur.close()conn.close()return customers

def load_week_schedule(username: str, customer_name: str, week: int) -> pd.DataFrame:conn = get_pg_conn()cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)cur.execute("""SELECT customer_name AS "לקוח",week AS "שבוע",day AS "יום",shift AS "משמרת",worker AS "עובד",created_at AS "נוצר בתאריך"FROM schedulesWHERE username=%s AND customer_name=%s AND week=%sORDER BY day, shift, worker""",(username, customer_name, week))rows = cur.fetchall()cur.close()conn.close()return pd.DataFrame(rows)

def load_all_schedules(username: str) -> pd.DataFrame:conn = get_pg_conn()cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)cur.execute("""SELECT customer_name AS "לקוח",week AS "שבוע",day AS "יום",shift AS "משמרת",worker AS "עובד",created_at AS "נוצר בתאריך"FROM schedulesWHERE username=%sORDER BY week DESC, day, shift, worker""",(username,))rows = cur.fetchall()cur.close()conn.close()return pd.DataFrame(rows)
...
def init_sqlite():conn = sqlite3.connect(DB_PATH)cur = conn.cursor()cur.execute("""CREATE TABLE IF NOT EXISTS users (username TEXT PRIMARY KEY,password_hash TEXT NOT NULL,salt TEXT NOT NULL,created_at TEXT DEFAULT CURRENT_TIMESTAMP)""")conn.commit()conn.close()

def hash_password(password: str, salt: str) -> str:dk = hashlib.pbkdf2_hmac("sha256", password.encode("utf-8"), salt.encode("utf-8"), 120_000)return dk.hex()

def create_user(username: str, password: str) -> bool:username = username.strip()if not username or not password:return Falsesalt = os.urandom(16).hex()p_hash = hash_password(password, salt)try:conn = sqlite3.connect(DB_PATH)cur = conn.cursor()cur.execute("INSERT INTO users(username, password_hash, salt) VALUES (?, ?, ?)", (username, p_hash, salt))conn.commit()conn.close()return Trueexcept sqlite3.IntegrityError:return False

def verify_user(username: str, password: str) -> bool:username = username.strip()conn = sqlite3.connect(DB_PATH)cur = conn.cursor()cur.execute("SELECT password_hash, salt FROM users WHERE username = ?", (username,))row = cur.fetchone()conn.close()if not row:return Falsestored_hash, salt = rowcheck_hash = hash_password(password, salt)return hmac.compare_digest(stored_hash, check_hash)

def auth_gate():init_sqlite()if "logged_in" not in st.session_state:st.session_state.logged_in = Falsest.session_state.username = ""
...
def simple_assignment(cost_matrix):used_rows, used_cols = set(), set()assignments = []rows = len(cost_matrix)cols = len(cost_matrix[0]) if rows > 0 else 0
...
def build_schedule(workers_df, req_df, pref_df, week_number):workers_df.columns = workers_df.columns.str.strip()req_df.columns = req_df.columns.str.strip()pref_df.columns = pref_df.columns.str.strip()
...
def safe_new_sheet_name(existing_names, base_name: str) -> str:if base_name not in existing_names:return base_namei = 2while True:candidate = f"{base_name} ({i})"if candidate not in existing_names:return candidatei += 1
...
st.sidebar.title("תפריט")page = st.sidebar.radio("ניווט", ["שיבוץ", "דשבורד", "מערכת מידע"], index=0)
...
if page == "שיבוץ":st.title("🧠 שיבוץ משמרות (Excel)")
...
elif page == "דשבורד":st.title("דשבורד")
...
elif page == "מערכת מידע":st.title("מערכת מידע")
